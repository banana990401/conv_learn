...
__global__ void implgemm(param_t param)
{
    ...
    __shared__ float smem_weight[8 * 132];
...
#pragma unroll
            for(int i = 0; i < 4; ++i)
            {
                weight_frag[i]     = smem_weight[weight_lds_addr + subcrs * 132 + i];
                weight_frag[i + 4] = smem_weight[weight_lds_addr + subcrs * 132 + i + 16];
            }
#pragma unroll
            for(int i = 0; i < 4; ++i)
            {
                input_frag[i]     = smem_input[input_lds_addr + subcrs * 128 + i];
                input_frag[i + 4] = smem_input[input_lds_addr + subcrs * 128 + i + 32];
            }
