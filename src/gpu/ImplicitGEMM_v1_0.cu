...
__global__ void implgemm(param_t param)
{
    ...
    for(int i = 0; i < param.c * param.r * param.s; i++)
    {
        int cur_c          = i / (param.r * param.s);
        int cur_r          = (i % (param.r * param.s)) / param.s;
        int cur_s          = (i % (param.r * param.s)) % param.s;
        int cur_h          = pos_h + cur_r;
        int cur_w          = pos_w + cur_s;
        int wei_offset_tmp = cur_c * wei_channel_offset + cur_r * param.s + cur_s;
        int in_offset_tmp  = cur_c * in_channel_offset + cur_h * param.w + cur_w;
        if(cur_h >= 0 && cur_w >= 0 && cur_h <= param.h && cur_w <= param.w)
        {
            sum +=
                param.weight[wei_offset + wei_offset_tmp] * param.input[in_offset + in_offset_tmp];
        }
    }
